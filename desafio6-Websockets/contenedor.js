...
class Contenedor {
    ...
    getAllMsgs = async () => {
        try {
            const archivoMsgs = await fs.promises.readFile(this.filePath2);
            const mensajes = JSON.parse(archivoMsgs);
            return (mensajes);
        } catch (e) {
            console.log(e);
        }
    ...
    saveMsgs = async (chat) => {
        try {
            const mensajes = await this.getAllMsgs();
            mensajes.push(chat);
            fs.promises.writeFile(
                this.filePath2,
